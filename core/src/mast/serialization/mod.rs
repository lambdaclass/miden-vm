...
pub fn read_from_old(source: &mut impl ByteReader) -> Result<MastForest, DeserializationError> {
        read_and_validate_magic(source).unwrap();
        read_and_validate_version(source).unwrap();
        ...
        let roots: Vec<u32> = Deserializable::read_from(source).unwrap();

        // Reading nodes
        let basic_block_data: Vec<u8> = Deserializable::read_from(source).unwrap();
        let mast_node_infos: Vec<MastNodeInfo> = node_infos_iter(source, node_count)
            .collect::<Result<Vec<MastNodeInfo>, DeserializationError>>().unwrap();
        let advice_map = AdviceMap::read_from(source).unwrap();

        // Reading Decorators
        let decorator_data: Vec<u8> = Deserializable::read_from(source).unwrap();
        let string_table: StringTable = Deserializable::read_from(source).unwrap();
        ...
        let mut mast_forest = {
            ...
            for decorator_info in decorator_infos {
                let decorator_info = decorator_info.unwrap();
                let decorator =
                    decorator_info.try_into_decorator(&string_table, &decorator_data).unwrap();
            ...
            for mast_node_info in mast_node_infos {
                let node =
                    mast_node_info.try_into_mast_node(node_count, &basic_block_data_decoder).unwrap();
            ...
            for root in roots {
                ...
                let root = MastNodeId::from_u32_safe(root, &mast_forest).unwrap();
        ...
        let basic_block_decorators: Vec<(usize, DecoratorList)> =
            read_block_decorators(source, &mast_forest).unwrap();
        for (node_id, decorator_list) in basic_block_decorators {
            let node_id = MastNodeId::from_usize_safe(node_id, &mast_forest).unwrap();
        ...
        let before_enter_decorators: Vec<(usize, Vec<DecoratorId>)> =
            read_before_after_decorators(source, &mast_forest).unwrap();
