use miden_processor::{ExecutionOptions, fast::DEFAULT_CORE_TRACE_FRAGMENT_SIZE};
...
fn test_cli_release_flag_controls_execution() {
    ...
    let debug_options = ExecutionOptions::new(DEFAULT_CORE_TRACE_FRAGMENT_SIZE, false, true)
        .expect("Default should enable debugging");
    assert!(debug_options.enable_debugging(), "CLI default should enable debug mode");

    let release_options = ExecutionOptions::new(DEFAULT_CORE_TRACE_FRAGMENT_SIZE, false, false)
    ...
    let trace_options = ExecutionOptions::new(DEFAULT_CORE_TRACE_FRAGMENT_SIZE, true, true)
